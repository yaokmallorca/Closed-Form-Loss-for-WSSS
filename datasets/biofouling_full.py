...
class Biofouling(Dataset):
    ...
    def __init__(self, root, data_root, img_transform = Compose([]),\
     label_transform=Compose([]), co_transform=Compose([]),\
      train_phase=True,split=1,labeled=True,seed=0,label_correction=False):
        ...
        self.label_root = os.path.join(self.data_root, 'BioFouling', 'AlphaLabels') #  SCRLabelsTrue_20 SUPLabels_80
    ...
    def __getitem__(self, index):
        ...
        with open(os.path.join(self.label_root,filename+'.png'), 'rb') as f:
            alpha_ch1 = load_image(f).convert('L')

        with open(os.path.join(self.label_root,filename+'_1.png'), 'rb') as f:
            alpha_ch2 = load_image(f).convert('L')

        image, alpha_ch1, alpha_ch2 = self.co_transform((image, alpha_ch1, alpha_ch2))
        image = self.img_transform(image)
        alpha_ch1 = self.label_transform(alpha_ch1)
        alpha_ch2 = self.label_transform(alpha_ch2)
        # return np.array(image), np.array(alpha_ch1), np.array(alpha_ch2), filename
        return image, alpha_ch1, alpha_ch2, filename
