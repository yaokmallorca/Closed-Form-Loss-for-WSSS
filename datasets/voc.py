...
class Voc(Dataset):
    ...
    def __init__(self, root, data_root, n_classes, img_transform = Compose([]),\
     label_transform=Compose([]), co_transform=Compose([]),\
      train_phase=True,split=1,labeled=True,seed=0,label_correction=False):
        ...
        self.palette_grayscale = [0, 1, 2, 15, 7, 12, 8] # [0, 38, 75, 147, 128, 33, 19]
    ...
    def _decode_mask(self, mask_org):
        ...
        mask[mask_org == 255] = 0
        mask[mask_org == 6] = self.palette_grayscale.index(7)
        mask[mask_org == 14] = self.palette_grayscale.index(2)
    ...
    def __getitem__(self, index):
        ...
        with open(os.path.join(self.eval_root,filename+'.png'), 'rb') as f:
            elabel = load_image(f).convert('P')
            elabel = elabel.resize((self.img_w, self.img_h))
            elabel = self._decode_mask(np.array(elabel))
            elabel = Image.fromarray(elabel)
            # print("elabel: ", np.unique(elabel))

        image_org = image.copy()
        if self.train_phase:
            image, trimap, elabel, image_org = self.co_transform((image, trimap, elabel, image_org))
        ...
        if self.train_phase:
            return np.array(image), np.array(trimap), np.array(image_org), elabel, clf_labels, filename
        else:
