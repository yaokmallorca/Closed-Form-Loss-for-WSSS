...
def evaluate_generator(Features = False):
    ...
    if args.val_orig:
        ...
        co_transform = transforms.Compose([ResizedImage3((512,512))])
    ...
    for img_id, (img, trimap, img_org, name) in enumerate(testloader):
        print(name)
        filename = os.path.join('results/closed_form/test/corrosion_unet', '{}.png'.format(name[0]))
        activation = {}
        ...
        solution = solution.reshape(512,512)
        sns.heatmap(solution, yticklabels=False, xticklabels=False)
        plt.savefig("corrosion_unet.png")
        input('s')
